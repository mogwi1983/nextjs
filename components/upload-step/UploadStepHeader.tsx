...
function UploadStepHeader({ projectId }: UploadStepHeaderProps) {
  const [browerFiles, setBrowserFiles] = useState<File[]>([]);
  const [uploading, setUploading] = useState(false);

  const inputFileRef = useRef<HTMLInputElement>(null);

  // TODO: Upload Files, handle files select, handle file drop
  ...
  const handleUpload = async () => {
    setUploading(true);
    try {
      const uploadPromises = browerFiles.map(async (file) => {
        const fileData = {
          projectId,
          title: file.name,
